...
function ukNav($items, $options = array()) {
	...
	if($items instanceof Page) {
		if(empty($header) && $header) $header = $items;
		$itemsParent = $items;
...
function ukNavbarNav(PageArray $items, $options = array()) {
	...
	$out = "<ul class='uk-navbar-nav $options[class]'>";
...
function ukSocialIcon($text) {
	$out = "";

	if(strpos($text, 'youtube') !== false) $out = "youtube";
	if(strpos($text, 'twitter') !== false) $out = "twitter";
	if(strpos($text, 'facebook') !== false) $out = "facebook";
	if(strpos($text, 'linkedin') !== false) $out = "linkedin";
